class ProfessorsController < ApplicationController
   ...
   if current_user.unidade_id == 53 or current_user.unidade_id == 52
        ...
        @classe_consulta = Classe.find(:all, :joins=> :unidade, :select => ['classes.id, CONCAT(classe_classe , " - " , unidades.nome) AS classe_classe '], :conditions=> ['classes.classe_ano_letivo =?' , Time.now.year], :order => 'classe_classe ASC, unidades.nome ASC')
        ...
        @classe_consulta = Classe.find(:all, :conditions => ['unidade_id = ? and classe_ano_letivo = ? ', current_user.unidade_id, Time.now.year  ], :order => 'classe_classe ASC')
    end
 end
                         ...
                         render :update do |page|
                           page.replace_html 'professores', :partial => "professores"
                                 ...
                                 if current_user.unidade_id == 52 or current_user.unidade_id == 53
                                   ...
                                   @professors = Professor.find( :all,:conditions => ["funcao like ? AND desligado = 0 and unidade_id=?" , "%" + params[:search].to_s + "%" , current_user.unidade_id],:order => 'nome ASC')
                                 end
                               render :update do |page|
                                      page.replace_html 'professores', :partial => "professores"
                               end
