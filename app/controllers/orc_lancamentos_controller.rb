...
def ficha_lancamento    #com SI
       ...
       @pagamentos = OrcPagamento.find(:all, :joins => 'LEFT JOIN orc_empenhos em ON em.id=orc_pagamentos.orc_empenho_id', :select => '"pg1" AS aaa,orc_pagamentos.id,(orc_pagamentos.data_pg)AS data_lanc, (orc_pagamentos.ficha)as rel_ficha, "O.P." as rel_tipo, (orc_pagamentos.codigo) AS rel_codigo, em.interessado AS rel_interessado,  (date_format(orc_pagamentos.data_pg,"%d/%m/%Y"))AS rel_data,"-o-"AS rel_valor_si, (em.valor_total) AS rel_valor_em, (valor_pg) AS rel_valor_op' , :conditions => ['orc_ficha_id = ?', params[:orc_ficha_id]], :order => 'data')
       @pag_sem_emp =                                                                              OrcPagamento.find(:all, :select => '"pg2" AS aaa,orc_pagamentos.id,(orc_pagamentos.data_pg)AS data_lanc, (orc_pagamentos.ficha)as rel_ficha, "O.P." as rel_tipo, (orc_pagamentos.codigo) AS rel_codigo, interessado    AS rel_interessado,  (date_format(orc_pagamentos.data_pg,"%d/%m/%Y"))AS rel_data,"-o-"AS rel_valor_si, "-o-"            AS rel_valor_em, (valor_pg) AS rel_valor_op' , :conditions => ['orc_ficha_id = ? AND orc_empenho_id IS NULL', params[:orc_ficha_id]])
       @lancamentos = (@pag_sem_emp)
       #.sort_by{|e| e[:data_lanc]}
       t0=0
