...
def ficha_lancamento    #com SI
       @ficha= OrcFicha.find(:all, :conditions => ['id = ?', params[:orc_ficha_id]])
       @pedidos = OrcPedidoCompra.find(:all, :joins => 'LEFT JOIN orc_fichas fichas ON orc_pedido_compras.orc_ficha_id=fichas.id LEFT JOIN orc_empenhos em ON em.orc_pedido_compra_id=orc_pedido_compras.id', :select => '(orc_pedido_compras.created_at)AS data_lanc, (fichas.ficha) AS rel_ficha,"S.I." as rel_tipo, (orc_pedido_compras.codigo)as rel_codigo, (orc_pedido_compras.fornecedor)AS rel_interessado, (date_format(orc_pedido_compras.created_at,"%d/%m/%Y"))AS rel_data, (orc_pedido_compras.valor_total)AS rel_valor_si, (em.valor_total) AS rel_valor_em, "-o-"AS rel_valor_op',  :conditions => ['orc_ficha_id = ?', params[:orc_ficha_id]])
       @empenhos = OrcEmpenho.find(:all, :select => '(data_chegou)AS data_lanc, ficha AS rel_ficha, "EMP." as rel_tipo, (codigo)AS rel_codigo , (interessado)AS rel_interessado,  (date_format(data_chegou,"%d/%m/%Y")) AS rel_data, "-o-"AS rel_valor_si, (valor_total)AS rel_valor_em, "-o-"AS rel_valor_op', :conditions => ['ficha = ?', @ficha[0].ficha])
       @pagamentos = OrcPagamento.find(:all, :joins => 'LEFT JOIN orc_empenhos em ON em.id=orc_pagamentos.orc_empenho_id', :select => '(orc_pagamentos.data_pg)AS data_lanc, (orc_pagamentos.ficha)as rel_ficha, "O.P." as rel_tipo, (orc_pagamentos.codigo) AS rel_codigo, em.interessado AS rel_interessado,  (date_format(orc_pagamentos.data_pg,"%d/%m/%Y"))AS rel_data,"-o-"AS rel_valor_si, (em.valor_total) AS rel_valor_em, (valor_pg) AS rel_valor_op' , :conditions => ['orc_ficha_id = ?', params[:orc_ficha_id]])
       @pag_sem_emp = OrcPagamento.find(:all, :select => '(orc_pagamentos.data_pg)AS data_lanc, (orc_pagamentos.ficha)as rel_ficha, "O.P." as rel_tipo, (orc_pagamentos.codigo) AS rel_codigo, interessado    AS rel_interessado,  (date_format(orc_pagamentos.data_pg,"%d/%m/%Y"))AS rel_data,"-o-"AS rel_valor_si, 0                AS rel_valor_em, (valor_pg) AS rel_valor_op' , :conditions => ['orc_ficha_id = ? AND orc_empenho_id IS NULL', params[:orc_ficha_id]])
       @lancamentos = (@pedidos + @empenhos + @pagamentos + @pag_sem_emp).sort_by{|e| e[:data_lanc]}
#       @lancamentos = (@pedidos + @empenhos + @pagamentos + @pag_sem_emp).sort_by{|e| e[:data_lanc]}
